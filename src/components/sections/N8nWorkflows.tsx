      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-20">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#ffffff]/10 border border-[#ffffff]/20 text-[#ffffff] text-xs font-black mb-6 tracking-widest uppercase"
          >
          ...
          >
            {fullPage ? (
              <>All <span className="text-[#737373]">n8n Workflows</span></>
            ) : (
              <>Our <span className="text-[#737373]">n8n Workflows</span></>
            )}
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            ...
            className="text-xl text-[#ffffff]/70 max-w-2xl mx-auto font-medium"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(fullPage ? n8nWorkflows : n8nWorkflows.slice(0, 6)).map((wf, index) => (
            <motion.div
              ...
              className="group bg-zinc-950 rounded-[2rem] border border-[#ffffff]/20 p-6 shadow-xl shadow-[rgba(255, 255, 255,0.08)] hover:shadow-2xl hover:shadow-[rgba(255, 255, 255,0.12)] hover:border-[#ffffff]/30 transition-all duration-300"
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                <Workflow className="w-5 h-5 text-[#ffffff] shrink-0 mt-0.5" />
                <span
                  className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                    wf.active ? "bg-green-500/15 text-green-600" : "bg-[#ffffff]/10 text-[#ffffff]/60"
                  }`}
                >
                  {wf.active ? "Active" : "Inactive"}
                </span>
              </div>
              <h3 className="text-lg font-black text-[#ffffff] mb-2 line-clamp-2 group-hover:text-[#e5e5e5] transition-colors">
                {wf.name}
              </h3>
              <p className="text-[#ffffff]/70 text-sm line-clamp-2 mb-3">{wf.description}</p>
              <p className="text-xs font-bold text-[#ffffff]/60">{wf.nodes} nodes</p>
            </motion.div>
          ))}
        </div>
        ...
        {!fullPage && n8nWorkflows.length > 6 && (
          <div className="mt-20 text-center">
            <motion.div
              ...
              whileHover={{ scale: 1.05, boxShadow: "0 8px 28px 0 rgba(255, 255, 255,0.1)" }}
              whileTap={{ scale: 0.97 }}
            ...
            >
              ...
              >
                ...
                <span className="absolute left-[-60%] top-0 h-full w-1/3 rotate-12 bg-zinc-900/30 opacity-0 group-hover:opacity-100 transition-all duration-700 group-hover:left-[110%] z-0" />
              </Link>
            </motion.div>
          </div>
        )}

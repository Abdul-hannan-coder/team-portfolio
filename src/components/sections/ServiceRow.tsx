...
export function ServiceRow({ service, index, isExpanded, onToggle, isLast }: ServiceRowProps) {
  ...
  return (
    <motion.article
      ...
      className={`relative ${!isLast ? "pb-16 md:pb-20" : ""}`}
    >
      <div
      ...
      >
        ...
        <div className={`flex-1 ${isReversed ? "md:text-right" : "md:text-left"} text-center`}>
          <div className={`flex flex-col ${isReversed ? "md:items-end" : "md:items-start"} items-center`}>
            ...
            <p className="mb-4 max-w-xl text-sm leading-relaxed text-white/65 sm:text-base">
              {service.description}
            </p>
            ...
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-semibold transition-all duration-300 hover:gap-2.5 sm:text-sm"
              style={{
                borderColor: "var(--accent-copper-border)",
        ...
        {/* Visual */}
        <div className="w-full flex-1 md:max-w-[48%]">
          <motion.div
            whileHover={{ y: -4 }}
            transition={{ type: "spring", stiffness: 400, damping: 28 }}
            className="group relative aspect-[16/10] max-h-[280px] overflow-hidden rounded-2xl border border-white/[0.08] bg-zinc-950 shadow-[0_24px_60px_-20px_rgba(0,0,0,0.85)] sm:max-h-[300px] lg:max-h-[320px]"
          >
            <div
              className="pointer-events-none absolute inset-0 z-10 rounded-2xl ring-1 ring-inset ring-white/[0.06]"
              aria-hidden
            />
            <div
      ...
      <motion.div
        ...
        className="absolute top-6 left-1/2 z-20 hidden h-10 w-10 -translate-x-1/2 items-center justify-center rounded-lg border md:flex"
        style={{
          borderColor: "var(--accent-copper-border)",
      ...
      >
        ...
        />
        <Icon className="relative z-10 h-4 w-4 text-black" />
      </motion.div>
    </motion.article>
  );
}
